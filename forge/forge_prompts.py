...
def build_act_prompt(task: dict, approved_plan: str) -> str:
    ...
    return (
        ...
        f"3. LINT: Run both clippy passes in order. Zero warnings permitted on either.\n"
        f"   a) `cargo clippy --workspace --features mock-hardware -- -D warnings`\n"
        f"      (mock-hardware path — all crates, scaffold and mock code)\n"
        f"   b) `cargo clippy --bin anvilml -- -D warnings`\n"
        f"      (real-hardware path — exercises #[cfg(unix)] and #[cfg(windows)] code\n"
        f"      that --features mock-hardware never compiles)\n"
        f"   Fix ALL warnings from both passes — including any pre-existing warnings\n"
        f"   not introduced by this task. Apply the most minimal correct solution for\n"
        f"   each (remove unused `mut`, add `_` prefix, apply the cargo fix suggestion).\n"
        f"   Never document a warning and skip it. List any pre-existing fixes in\n"
        f"   ## Deviations from Plan. Zero warnings required before proceeding.\n"
        f"4. PLATFORM CROSS-CHECK: Run all three checks in order. Zero errors required.\n"
        f"   a) `cargo check --target x86_64-pc-windows-gnu --workspace --features mock-hardware`\n"
        f"      (mock-hardware Windows-gnu cross-check)\n"
        f"   b) `cargo check --bin anvilml`\n"
        f"      (real-hardware Linux native — exercises #[cfg(unix)] paths)\n"
        f"   c) `cargo check --bin anvilml --target x86_64-pc-windows-gnu`\n"
        f"      (real-hardware Windows-gnu cross-check — exercises #[cfg(windows)] paths)\n"
        ...
        f"       giving each test its own independent state (unique TempDir, unique port,\n"
        f"       unique in-memory fixture). Do NOT use #[serial] or --test-threads=1\n"
        f"       unless the resource is physically singular (e.g. a hardware device);\n"
        f"       if you must, justify it in ## Deviations from Plan.\n"
        f"   (b) True flakiness (timing, network) must be documented in ## Test Results\n"
        f"       with root cause identified; the final recorded run must show 0 failures.\n"
